...
export function createMesh(THREE) {
  ...
  const titleBadge = createTextBadge(THREE, "Mapbox GL JS: Mapas Vetoriais 3D & Camadas WebGL", "#38bdf8", 480, 60);
  titleBadge.position.set(0, 1.7, 0);
  titleBadge.scale.set(2.4, 0.3, 1);
  group.add(titleBadge);

  const statusBadge = createTextBadge(THREE, "Extrusão de Edifícios Urbanos (Altura) + Pulso Radar GPS", "#fbbf24", 510, 60);
