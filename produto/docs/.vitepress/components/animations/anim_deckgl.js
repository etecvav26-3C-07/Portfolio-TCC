...
export function createMesh(THREE) {
  ...
  const titleBadge = createTextBadge(THREE, "Deck.gl: Visualização Geoespacial em Larga Escala", "#38bdf8", 480, 60);
  titleBadge.position.set(0, 1.7, 0);
  titleBadge.scale.set(2.4, 0.3, 1);
  group.add(titleBadge);

  const statusBadge = createTextBadge(THREE, "HexagonLayer (Agregação Espacial) + ArcLayer (Fluxos O-D)", "#fbbf24", 510, 60);
