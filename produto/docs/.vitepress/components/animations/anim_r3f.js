...
export function createMesh(THREE) {
  ...
  // Badges conceituais
  const titleBadge = createTextBadge(THREE, "React Three Fiber (R3F): Átomo Declarativo Reativo", "#38bdf8", 480, 60);
  titleBadge.position.set(0, 1.7, 0);
  titleBadge.scale.set(2.4, 0.3, 1);
  group.add(titleBadge);

  const statusBadge = createTextBadge(THREE, "<Canvas> → <mesh> → <boxGeometry> + <meshStandardMaterial>", "#fbbf24", 500, 60);
