...
export function createMesh(THREE) {
  ...
  const titleBadge = createTextBadge(THREE, "A-Frame & WebXR: Arquitetura ECS (Entity • Component • System)", "#38bdf8", 500, 60);
  titleBadge.position.set(0, 1.7, 0);
  titleBadge.scale.set(2.5, 0.3, 1);
  group.add(titleBadge);

  const statusBadge = createTextBadge(THREE, "Rastreamento 6DoF • Raycasting Interativo • HTML Declarativo", "#fbbf24", 500, 60);
